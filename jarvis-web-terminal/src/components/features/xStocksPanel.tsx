...
export function xStocksPanel({ stocks, isLoading }: xStocksPanelProps) {
    ...
    return (
        <div className="sentiment-panel">
            <div className="sentiment-panel-header">
                <LineChart className="w-5 h-5 text-accent-neon" />
            ...
            <div className="space-y-2">
                {stocks.map((stock, i) => (
                    ...
                    >
                        {/* Direction Icon */}
                        <div className={`p-2 rounded-lg ${stock.direction === 'LONG' ? 'bg-emerald-500/20' : 'bg-accent-error/20'}`}>
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                                ...
                                <span className={`px-1.5 py-0.5 text-xs rounded ${stock.direction === 'LONG' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-accent-error/20 text-accent-error'}`}>
                                    {stock.direction}
                                </span>
                            </div>
                        ...
                        <div className="text-right">
                            ...
                            <div className="text-xs text-accent-error">SL: {stock.stopLoss}</div>
                        </div>
                    </div>
                ))}
