...
must_haves:
  truths:
    - "SQLite database jarvis.db is created with WAL mode enabled"
    - "All required tables exist (facts, entities, entity_mentions, preferences, sessions)"
    - "FTS5 virtual table facts_fts is created with porter unicode61 tokenizer"
    - "fact_embeddings table links SQLite facts to PostgreSQL IDs"
    - "Triggers automatically sync facts table to FTS5 index"
  artifacts:
...
- jarvis.db created at ~/jarvis/memory/ with all tables
</objective>
...
def init_database(db_path: Optional[Path] = None) -> DatabaseManager:
    ...
    Args:
        db_path: Optional path override. Uses ~/jarvis/memory/jarvis.db by default.
...
Should show database path, schema version 1, all tables, and WAL mode.
  ...
  <done>
- jarvis.db created at ~/jarvis/memory/jarvis.db
...
<success_criteria>
- jarvis.db file exists at ~/jarvis/memory/jarvis.db
