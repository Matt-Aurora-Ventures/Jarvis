...
must_haves:
  truths:
    - "Memory workspace exists at ~/.lifeos/memory/ with all required subdirectories"
    ...
    - "Path ~/.lifeos/memory/ is used per INT-006 to coexist with existing trading state"
  artifacts:
...
<objective>
Create the unified memory workspace directory structure at ~/.lifeos/memory/ (per INT-006, coexisting with existing trading state) with all required subdirectories for the dual-layer Markdown + SQLite architecture.
...
- Directory structure created at ~/.lifeos/memory/
</objective>
...
```
~/.lifeos/memory/               # Memory workspace root
...
Should show all directories exist and memory.md was created.
  ...
  <done>
- init_workspace() creates ~/.lifeos/memory/ with all subdirectories
...
```bash
ls -la ~/.lifeos/memory/
ls -la ~/.lifeos/memory/memory/
ls -la ~/.lifeos/memory/bank/
ls -la ~/.lifeos/memory/bank/entities/
```

3. Verify placeholder files:
```bash
head -5 ~/.lifeos/memory/memory.md
head -5 ~/.lifeos/memory/SOUL.md
```
</verification>

<success_criteria>
- core/memory/ package imports without errors
- ~/.lifeos/memory/ directory exists with all subdirectories
