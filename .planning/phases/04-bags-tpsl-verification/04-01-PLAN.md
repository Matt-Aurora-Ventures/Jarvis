...
1. **BagsAPIClient** (lines 77-559):
   - ✓ API key + partner key configuration (lines 87-111)
   - ✓ get_quote() method (lines 168-213)
   - ✓ swap() method with partner fee (lines 215-310)
   - ✓ get_token_info() (lines 312-351)
   - ✓ Rate limiting (60 req/min) (lines 138-153)
   - ✓ Success/failure tracking (lines 280, 300)

2. **BagsTradeRouter** (lines 562-686):
   - ✓ Bags.fm primary, Jupiter fallback pattern (lines 600-646)
   - ✓ Trade statistics tracking (lines 671-685)

3. **SuccessFeeManager** (lines 688-805):
   - ✓ 0.5% fee on profitable trades (line 696)
   - ✓ calculate_success_fee() (lines 703-746)
   - ✓ collect_fee() (lines 748-795)

**File**: `core/treasury/bags_integration.py` (463 lines)

**Components Found**:
1. **TreasuryBagsIntegration** (lines 59-398):
   - ✓ Automated fee collection scheduling (lines 261-296)
   - ✓ Fee distribution: 50% staking, 30% ops, 20% dev (lines 22-26)
   - ✓ Revenue analytics (lines 332-397)

**File**: `tg_bot/handlers/demo/demo_orders.py` (432 lines)

**TP/SL Components Found** (✓ VERIFIED lines 1-150):
1. **_check_demo_exit_triggers()** (lines 71-155):
   - ✓ Take-profit checking (lines 116-122)
   - ✓ Stop-loss checking (lines 124-130)
   - ✓ Trailing stop updating (lines 132-150)
...
```bash
BAGS_API_KEY=bags_prod_X4VozAQVsxlu8KMwPG5v3NE3-LdiHKMvuFyRM0CE07k
BAGS_PARTNER_KEY=bags_prod_X4VozAQVsxlu8KMwPG5v3NE3-LdiHKMvuFyRM0CE07k
...
3. **Integration Testing**: No evidence of E2E bags.fm → TP/SL flow
...
async def test_bags_api():
    ...
    if quote:
        print(f"✓ Quote: {quote.from_amount} SOL -> {quote.to_amount} USDC")
    ...
    if "error" not in stats:
        ...
        print(f"✗ Partner stats failed: {stats.get('error')}")
...
1. **demo_trading.py::execute_buy_with_tpsl()**
   - ✓ Already has TP/SL in signature
...
async def execute_buy(
    token: str,
    amount: float,
    tp_percent: Optional[float] = None,  # ❌ Optional
    sl_percent: Optional[float] = None,  # ❌ Optional
):
    # ...
```
...
async def test_monitoring():
    ...
    if tpsl_jobs:
        print(f"✓ Found {len(tpsl_jobs)} TP/SL monitoring jobs")
        for job in tpsl_jobs:
            print(f"  - {job.name}: interval={job.interval}s")
    else:
        print("✗ No TP/SL monitoring jobs found!")
...
**Objective**: Test full bags.fm → TP/SL flow end-to-end
...
# After
"⚠️ bags.fm API temporarily unavailable. Retrying with Jupiter fallback..."
```
...
# After
"❌ bags.fm API key invalid or expired. Check BAGS_API_KEY in settings. Trade cancelled for safety."
```
...
Try bags.fm (BagsTradeRouter)
    ├─ Success → Record position with TP/SL
    └─ Failure → Fallback to Jupiter
            ↓
    Background Monitor (every 30s)
        ↓
    Check TP/SL triggers
        ├─ TP hit → Alert + Auto-exit (if enabled)
        ├─ SL hit → Alert + Auto-exit (if enabled)
        └─ Trailing stop hit → Alert + Auto-exit
```
...
**Critical Path**: Task 1 → Task 2 → Task 3 (verification before enhancement)
