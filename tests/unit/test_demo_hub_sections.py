...
async def test_demo_hub_prestocks_graceful_message():
    ...
    with patch("tg_bot.handlers.demo.demo_core.get_config", return_value=config), \
        patch("tg_bot.handlers.demo.demo_core.get_market_regime", new=AsyncMock(return_value={"regime": "BULL"})), \
        patch("tg_bot.handlers.demo.demo_core._get_demo_engine", new=AsyncMock(return_value=DummyEngine())):
