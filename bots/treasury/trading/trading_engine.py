...
class TradingEngine(TradingOperationsMixin):
    ...
    from .constants import (
        ...
        ESTABLISHED_TOKENS as ESTABLISHED_TOKENS,
        HIGH_RISK_PATTERNS as HIGH_RISK_PATTERNS,
        BLOCKED_TOKENS as BLOCKED_TOKENS,
        BLOCKED_SYMBOLS as BLOCKED_SYMBOLS,
        MIN_VOLUME_24H_USD as MIN_VOLUME_24H_USD,
        MIN_TOKEN_AGE_HOURS as MIN_TOKEN_AGE_HOURS,
        MAX_HIGH_RISK_POSITION_PCT as MAX_HIGH_RISK_POSITION_PCT,
        MAX_UNVETTED_POSITION_PCT as MAX_UNVETTED_POSITION_PCT,
    )
    ...
    def get_risk_status(self) -> Optional[Dict[str, Any]]:
        ...
        # Get portfolio value - handle async/sync boundary properly
        try:
            import asyncio
            # Check if we're already in an async context
            try:
                loop = asyncio.get_running_loop()
                # Already in async context - use asyncio.create_task or similar
