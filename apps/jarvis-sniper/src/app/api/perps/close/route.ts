...
import { fallbackClosePosition } from '@/lib/perps/fallback-runtime';
...
export async function POST(request: Request) {
  ...
  const upstream = await proxyPerpsPost('/close', request);
  if (upstream.status < 500) return upstream;

  const fallback = fallbackClosePosition(normalizedPayload);
