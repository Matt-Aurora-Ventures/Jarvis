...
import { fallbackUpdateLimits } from '@/lib/perps/fallback-runtime';
...
export async function POST(request: Request) {
  ...
  const upstream = await proxyPerpsPost('/limits', request);
  if (upstream.status < 500) return upstream;

  const fallback = fallbackUpdateLimits(normalizedPayload);
