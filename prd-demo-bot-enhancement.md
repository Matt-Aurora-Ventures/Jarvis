...
Transform the Telegram /demo bot into an **AI-powered trading assistant** that combines:
- **Real-time sentiment analysis** from Grok AI (updated every 15 minutes)
- **bags.fm API** for trading execution
- **Treasury activation signals** with live PnL tracking
- **Self-learning AI** that compresses observations and improves over time
- **Beautiful, responsive UI** matching the screenshot design
- **Advanced order management** (stop-loss, take-profit, limit orders)

This is not just a trading bot—it's an **AI trading copilot** that learns from market behavior, user actions, and execution results to continuously improve its recommendations.

---

## Problem Statement

The current /demo bot has fundamental gaps preventing public launch:

### Technical Issues
1. **Buy/sell flows broken** - message handler not registered (tg_bot/bot.py:181)
2. **Wallet not initialized** - demo profile lacks wallet setup
3. **Charts non-functional** - no chart generation or caching
4. **Advanced orders missing** - no stop-loss, take-profit, or limit orders

### Feature Gaps
5. **No sentiment integration** - rich sentiment data exists but isn't exposed in demo bot
6. **No AI learning** - observations from trades aren't compressed or used to improve
7. **No treasury signals** - treasury activation data not shown to users
8. **UI lacks polish** - missing the comprehensive menu structure shown in screenshot

### User Experience Issues
9. **Buy buttons unexplained** - users don't understand why preset amounts exist or what they do
10. **No onboarding** - new users dropped into complex interface without guidance
11. **Poor error messages** - raw exceptions instead of actionable guidance
12. **No self-correction** - failures require manual intervention
...
| Goal | Success Metric |
|------|---------------|
| **Working Trades** | 100% buy/sell completion rate |
| **AI Intelligence** | Sentiment updates every 15 min, observations compressed hourly |
| **User Engagement** | 80%+ users complete first trade within 5 minutes |
| **Reliability** | 99.9% uptime, < 1% API error rate |
| **Learning** | AI accuracy improves 10% month-over-month |
| **Public Ready** | Pass security audit, handle 1000+ concurrent users |

---

## User Stories

### 🎯 CRITICAL PATH: Buy Flow
...
*[Due to length, I'll add abbreviated versions of remaining stories]*
...
│  │  - Learning compressor          │   │
│  │  - Sentiment updater (15 min)   │   │
│  │  - Order monitor (10 sec)       │   │
...
|--------|--------|-------------|
| **Buy Flow Completion** | 100% | No handler errors |
| **Sentiment Update SLA** | 99% on-time | Updates hit 15-min mark |
| **Order Execution Accuracy** | 99.9% | Orders execute within 1% of trigger |
| **AI Learning Accuracy** | 60%+ prediction rate | Learnings vs outcomes |
| **User Engagement** | 80% complete first trade | Within 5 minutes |
| **System Uptime** | 99.9% | Downtime < 43 min/month |
| **API Error Rate** | < 1% | bags.fm API calls |
| **Chart Load Time** | < 3s | 95th percentile |
| **WebSocket Latency** | < 100ms | Price updates |
| **User Satisfaction** | 4.5/5 stars | Post-trade survey |
...
- **US-033: Critical Bug Fixes** (P0 - BLOCKER)
  - Fix safe_symbol NameError
  - Fix amount KeyError
  - Fix bot instance conflicts
  - Wire TP/SL adjustment UI
- **US-032: Loading States & Menu Transitions** (P0 - UX Critical)
- US-001: Message handler registration ✅
- US-002: Sentiment hub UI
- **US-005: bags.fm buy/sell + Jupiter backup + TP/SL** (Enhanced)
- US-009: Wallet initialization

### Phase 2: Intelligence & Charts (Week 2)
- US-003: AI learning system
- US-004: Treasury integration
- US-008: 15-minute updates
- **US-007: Charts with mplfinance** (Enhanced)

### Phase 3: Risk Management & Custom Features (Week 3)
- US-006: Stop-loss/take-profit (additional UI)
- **US-031: Custom Buy Amount Input** (P1 - User requested)
- US-016: Quick trade
- US-018: Positions dashboard
- US-027: Alerts
...
- US-010: UI redesign
- US-011-015: Quick actions (Insta Snipe, AI Picks, etc.)
- US-021: PnL reports
- US-025: Learning dashboard
...
```
core/
  bags_api.py                    # bags.fm API client
  jupiter_api.py                 # Jupiter fallback API client (US-005)
  retry_decorator.py             # Retry logic

tg_bot/handlers/
  demo_sentiment.py              # Sentiment hub UI
  demo_treasury.py               # Treasury activation UI
  demo_charts.py                 # Chart generation (mplfinance-based, US-007)
  demo_orders.py                 # Order management UI
  demo_wallet.py                 # Wallet management
  demo_health.py                 # Health monitoring

tg_bot/services/
  observation_collector.py       # Log observations
  learning_compressor.py         # Hourly AI compression
  insight_engine.py              # Apply learnings
  sentiment_updater.py           # 15-min sentiment updates
  order_monitor.py               # Monitor orders (10s)
  treasury_monitor.py            # Monitor treasury (real-time)
  health_monitor.py              # System health checks

tg_bot/utils/
  loading.py                     # Loading indicators (US-032)
  error_formatter.py             # User-friendly errors

tests/
  test_demo_bot.py               # Integration tests
  test_demo_bug_fixes.py         # Regression tests for US-033 bug fixes
  fixtures/bags_api_mocks.py     # API mocks

~/.lifeos/trading/
  demo_positions.json            # User positions
  demo_orders.json               # Active orders
  demo_observations.jsonl        # Raw observations
  demo_learnings.json            # Compressed insights
  demo_transactions.json         # Trade history
  demo_preferences.json          # User settings
  demo_portfolio_history.json    # PnL snapshots
```

### Modified Files
```
tg_bot/bot.py                    # Add message handler registration
tg_bot/handlers/demo.py          # Critical changes:
                                 # - Fix safe_symbol NameError (US-033)
                                 # - Fix amount KeyError (US-033)
                                 # - Add custom buy amount handler (US-031)
                                 # - Add loading indicators (US-032)
                                 # - Wire TP/SL adjustment callbacks (US-033)
                                 # - Add execute_buy_with_tpsl (US-005)
                                 # - MenuNavigator integration (US-032)
tg_bot/ui/inline_buttons.py     # Add "Custom Amount" button (US-031)
tg_bot/ui/interactive_menus.py  # Ensure MenuNavigator exported (US-032)
bots/supervisor.py               # Single instance enforcement (US-033)
                                 # Start background services
bots/buy_tracker/sentiment_report.py  # Export data for demo bot
requirements.txt                 # Add mplfinance, pandas (US-007)
```
...
- bags.fm API: https://docs.bags.fm/
- Sentiment Report: `bots/buy_tracker/sentiment_report.py`
- Treasury Trading: `bots/treasury/trading.py`
- Bags Intel: `bots/bags_intel/intel_service.py`
- Current Demo Handler: `tg_bot/handlers/demo.py`
- Jarvis Context: `CLAUDE.md`

---

## Open Questions

1. Should we support testnet for demo bot or require real funds?
2. What's the bags.fm API rate limit? Need to avoid hitting it.
3. Should AI learnings be user-specific or global (all users)?
4. Max number of active orders per user?
5. Should charts use matplotlib or send links to TradingView-style embeds?
6. Do we need a separate demo wallet per user or shared demo profile?
7. What's the target for production: 100 users? 1000? 10K?
8. Should we add social features (leaderboard, shared trades)?
