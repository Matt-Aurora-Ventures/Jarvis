...
// ============================================================
// Position persistence — save to server for risk worker
// ============================================================

export async function savePositionToServer(position: {
  mint: string;
  symbol: string;
  amount: number;
  amountLamports?: string;
  ...
  try {
    ...
    console.warn('[Positions] Failed to persist position to server:', err);
  }
}

export async function closePositionOnServer(params: {
  mint: string;
  status?: 'closed' | 'sl_hit' | 'tp_hit' | 'trail_stop' | 'expired';
  txHash?: string;
}): Promise<void> {
  try {
    const mint = encodeURIComponent(params.mint);
    const status = encodeURIComponent(params.status || 'closed');
    const txHash = params.txHash ? `&txHash=${encodeURIComponent(params.txHash)}` : '';
    await fetch(`/api/positions?mint=${mint}&status=${status}${txHash}`, { method: 'DELETE' });
  } catch {
    // Best-effort persistence; UI/worker should not crash on this.
  }
}
