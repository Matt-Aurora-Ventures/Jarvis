...
export interface BlueChipToken {
  ...
  name: string;
  mintAddress: string;
  category: 'defi' | 'infra' | 'meme_established' | 'gaming' | 'ai' | 'lsd';
  description: string;
  /** Approximate daily volatility % (average absolute daily move) */
  avgDailyVolatility: number;
  /** Approximate market cap tier */
  mcapTier: 'mega' | 'large' | 'mid';
  /** Jupiter Strict List verified */
  jupiterStrict: true;
  /** How many years on Solana */
  yearsSolana: number;
}
...
export const BLUECHIP_TIER1: BlueChipToken[] = [
  {
    ticker: 'SOL',
    name: 'Wrapped SOL',
...
export const BLUECHIP_TIER2: BlueChipToken[] = [
  {
    ticker: 'ORCA',
    name: 'Orca',
...
export const BLUECHIP_TIER3: BlueChipToken[] = [
  {
    ticker: 'BONK',
    name: 'Bonk',
...
export const ALL_BLUECHIPS: BlueChipToken[] = [
  ...BLUECHIP_TIER1,
  ...BLUECHIP_TIER2,
  ...BLUECHIP_TIER3,
];

export const BLUECHIP_BY_MINT = new Map(
  ALL_BLUECHIPS.map(t => [t.mintAddress, t])
);

export const BLUECHIP_BY_TICKER = new Map(
  ALL_BLUECHIPS.map(t => [t.ticker, t])
);
