...
def load_signer_keypair(expected_wallet_address: str = "") -> Keypair:
    ...
    keypair_path = os.environ.get("PERPS_SIGNER_KEYPAIR_PATH", "").strip()
    ...
    if keypair_b58:
        ...
        if not source.exists():
            raise RuntimeError(f"PERPS_SIGNER_KEYPAIR_PATH does not exist: {source}")
        key_material = _decode_keypair_material(source.read_text(encoding="utf-8"))
    else:
        raise RuntimeError(
            "Live mode requires PERPS_SIGNER_KEYPAIR_B58 or PERPS_SIGNER_KEYPAIR_PATH"
        )
